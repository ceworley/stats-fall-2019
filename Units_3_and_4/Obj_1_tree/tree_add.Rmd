...
n = sample(2:4)
x = sample(2:5,n,replace=T)

ords = c("first","second","third","fourth","fifth","sixth")
s = ''
for(i in 1:(length(x)-1)){
    s = paste(s, 'the', ords[i], 'basket has', x[i], 'widgets,')
}
s = paste(s, 'and the', ords[n], 'basket has', x[n], 'widgets.')
...
`r na` has `r n` baskets of widgets; `r s`. How many total widgets are there?
