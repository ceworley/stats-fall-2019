...
if(pvalue < alpha){
  ...
  sym = "<"
  ans = "We conclude the die is unfair, with a lower than fair expected value."
} else {
...
You are told to check if a `r sides`-sided die has an expected value less than `r mu`. You are told to roll the die `r n` times and do a one-tail significance test with a significance level of `r alpha`.
