...
library(latex2exp)
options(scipen=999)
otalphas = c(1,0.1, 0.05, 0.025, 0.02, 0.01, 0.005, 0.0025, 0.002, 0.001,0)
alphas = c(0.05,0.02,0.01)  #,0.005,0.002,0.001)
...
tstar = abs(qt(ell,df))
mu = sample(50+(0:20), 1)
sig = round(runif(1,mu/10,mu/4),2)
...
while(T){
  ...
  pvalue = round(1-pt(abs(t),df),4)
  if(abs(t-tstar)<0.2 & abs(t-tstar)>0.02){
    break
  }
}
lbpv = max(otalphas[otalphas<pvalue])
ubpv = min(otalphas[otalphas>pvalue])
if(pvalue < alpha){
...
A null hypothesis claims a population has a mean $\mu = `r mu`.0$. You decide to run right-tail test on a sample of size $n=`r n`$ using a significance level $\alpha = `r alpha`$. 
...
Find the mean and standard deviation of the sample (use calculator function).
$$\bar{x} = `r xbar` $$
$$s = `r s` $$
...
\
```{r pvalue_sketch_rttraw, echo = FALSE, results = "hide", fig.height = 3, fig.width = 7, fig.cap = ""}
par(mar=c(5,3,1,1))
top = qt(0.999,df)
bot = -top
tt = floor(top)
...
$$P(T > `r round(qt(abs(1-lbpv),df),2)`) = `r lbpv` $$
$$P(T > `r round(qt(abs(1-ubpv),df),2)`) = `r ubpv` $$
Basically, because $t$ is between `r round(qt(abs(1-lbpv),df),2)` and `r round(qt(abs(1-ubpv),df),2)`, we know the $p$-value is between `r lbpv` and `r ubpv`.
...
Answerlist
--------
* The hypotheses: $H_0$ claims $\mu=`r mu`$ and $H_A$ claims $\mu>`r mu`$.
