...
$$P(T > `r round(qt(-abs(lbpv),df),2)`) = `r lbpv` $$
$$P(T > `r round(qt(-abs(ubpv),df),2)`) = `r ubpv` $$
Basically, because $t$ is between `r round(qt(-abs(lbpv),df),2)` and `r round(qt(-abs(ubpv),df),2)`, we know the $p$-value is between `r lbpv` and `r ubpv`.
