...
ft = ""
for(i in 1:nc){
  ft = paste(ft,"|",myhist$breaks[i],"--",myhist$breaks[i+1],"|",myhist$counts[i],"|\n",sep="")
}
...
A continuous random variable was measured `r n` times. The resulting frequency distribution is shown below.
...
* What percent of the measurements are between `r min(b1,b2)` and `r max(b1,b2)`?
* What percent of the measurements are within `r dis` of `r cen`? In other words, what percent of measurements satisfy $|x-`r cen`| \le `r dis`$?
* Of the measurements `r t1` than `r b1`, what percent are `r t2` than `r b2`?
* Estimate the value of the `r signif(p4*100,4)`th percentile. In other words, determine a value such that `r signif(p4*100,4)`% of the measurements are less than or equal to it.
...
* `r signif(p1,4)*100`%
* `r signif(p2,4)*100`%
* `r signif(sum(myhist$counts[min(s1,s2):(max(s1,s2)-1)])/n,4)*100`%
* `r signif(pp,4)*100`%
* `r signif(p3,4)*100`%
* `r b4`
...
============
extype: cloze
exsolution: `r type`|`r max(myhist$breaks)-min(myhist$breaks)`|`r signif(p1,4)`|`r signif(p2,4)`|`r signif(sum(myhist$counts[s1:(s2-1)])/n,4)`|`r signif(pp,4)`|`r signif(p3,4)`|`r b4`
