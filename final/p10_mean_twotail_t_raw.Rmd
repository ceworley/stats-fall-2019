...
tstar = abs(qt(ell,df))
mu = sample(5+(0:20), 1)
sig = round(runif(1,mu/10,mu/3),2)
...
A null hypothesis claims a population has a mean $\mu = `r mu`.0$. You decide to run two-tail test on a sample of size $n=`r n`$ using a significance level $\alpha = `r alpha`$. 
