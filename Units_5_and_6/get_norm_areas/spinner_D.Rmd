...
makepie = function(p1,p2){ #,name){
  ...
  if(p1<p2){
    ...
    lines(c(cos(p2*2*pi)*0.7,cos(p2*2*pi)*0.83),
          ...
          lwd = 2)
    text(cos(p1*2*pi),sin(p1*2*pi),round(qnorm(p1),2))
    text(cos(p2*2*pi),sin(p2*2*pi),round(qnorm(p2),2))
  }
  if(p1>p2){
...
qs = c(qs,paste("Evaluat e $P(|Z|<",z,")$",sep=""))
...
```{r a,echo=F,fig.height=5,fig.width=5,fig.cap="",path=paste("figs/hub",sample(1:999999,1),sep="")}
par(mfrow=c(3,3))
for(i in 1:9){
  makepie(ppp[ord[i]*2-1],ppp[ord[i]*2])
  mtext(c("a","b","c","d","e","f","g","h","i")[i],side = 3)}
```
\
