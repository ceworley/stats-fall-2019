...
* Estimate $x$ such that $P(X<x) = `r p1`$ by shading and counting.
* Determine $x$ such that $P(X<x) = `r p1`$ by using the $z$-table.
...
\
\newline
When you have shaded `r round(pnorm(z),2)*100` squares, starting on the left, you should end around $x=`r z*si+mu`$.
* $x \approx `r round(qnorm(p1),2)*si+mu`$ 
