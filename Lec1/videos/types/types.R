...
getp = function(ps,framenum,fpspin=20,fpstop=10,rps=3){
...
set.seed(123)
p1 = runif(10)
for(i in 1:(length(p1)*30)){
  png(sprintf("pngs/%s%03d.png",fn,i),width=400,height=400)
  par(mar=c(0,0,0,0))
  plot(1, 1, type="n", axes=FALSE, ann=FALSE, xlim=c(0,1), ylim = c(0,1))
  fig.align="center"
  draw_uniform()
  arrow(p=getp(p1,i))
  draw_marks2(p1[0:((i+10)%/%30)])
  dev.off()
}
